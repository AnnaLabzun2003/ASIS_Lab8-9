...
Landmark.update = function (id, guide, result){
    connection.query("UPDATE dovid_landmark SET landmark_name = ? WHERE id_landmark = ?", [guide.guide_name, id], function(err, res){
        if (err){
            console.log("error: ", err);
            result(err, null);
        }
